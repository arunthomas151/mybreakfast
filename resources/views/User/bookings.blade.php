    ...
    <section class="section-sm row bg-white row">
        ...
        <table>
            ...
            <tr>
                ...
                <td>
                    <div class="col-md-6">
                        <img src="images/room/{{ $rbookings->nrimage}}" alt="" width="370" height="244" />
                    </div>
                </td>
                <td>{{ $rbookings->cidate}}</td>
                <td>{{ $rbookings->codate}}</td>
                <td>{{ $rbookings->nop }}</td>
                <td>{{ $rbookings->amount }}</td>
                <td>
                    @if($rbookings->cidate > $todaycurrent)
                    <a class="button button-icon button-primary cancel-rooms" href="#" data-toggle="modal"
                        data-target="#cancelbook" data-rbkid="{{ $rbookings->rbid }}"><span>Cancel</span><span
                            class="icon material-icons-chevron_right"></span></a></td>
                {{-- @elseif($rbookings->cidate == $todaycurrent) --}}
        ...
        <table>
            ...
            <tr>
                ...
                <td>
                    <div class="col-md-6">
                        <img src="images/room/{{ $obookings->nrimage}}" alt="" width="370" height="244" />
                    </div>
                </td>
                <td>{{ $obookings->cidate}}</td>
                <td>{{ $obookings->codate}}</td>
                <td>{{ $obookings->nop }}</td>
                <td>{{ $obookings->amount }}</td>
                <td>
                    @if($rbookings->codate >= $todaycurrent)
                    <a class="button button-icon button-primary rating-rooms" href="#" data-toggle="modal"
                        data-target="#rateroom" data-rbkid1="{{ $obookings->rbid }}"
                        data-rimager="{{ $obookings->nrimage }}" data-rtbkid="{{ $obookings->nrid }}">
                        <span>Rating</span><span class="icon material-icons-chevron_right"></span></a></td>
                @endif
...
<div class="modal login" id="cancelbook">
                ...
                <div class="box">
                    <div class="content">
                        <div class="form loginBox">
                            <form method="post" action="/rbookcancel" accept-charset="UTF-8"
                                enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="biikid" id="bookid">
...
<div class="modal login" id="rateroom">
    <div class="modal-dialog login animated">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
            ...
            <div class="modal-body">
                <div class="box">
                    <div class="content">
                        <div class="form loginBox">
                            <form action="/roomrating" method="post" accept-charset="UTF-8"
                                enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="roomid1" id="roomid1">
                                <input type="hidden" name="roombkid" id="roombkid">
                                <input id="ratings-hidden" name="rating" type="hidden" value="1">
                                <div class="col-md-6">
                                    {{-- <img src="" id="rrimagert" class="img-circle img-responsive"/> --}}
                                    <img src="" id="rrimagert" />
                                </div>
...
<script>
    $(".cancel-rooms").on("click", function () {
        $bkid = $(this).data("rbkid");
        $("#bookid").val($bkid);
    });

    $(".rating-rooms").on("click", function () {
        $rbkid = $(this).data("rtbkid");
        $("#roomid1").val($rbkid);
        $bkid1 = $(this).data("rbkid1");
        $("#roombkid").val($bkid1);
        $rrimage = $(this).data("rimager");
        $("#rrimagert").attr("src", "images/room/" + $rrimage);
    });
