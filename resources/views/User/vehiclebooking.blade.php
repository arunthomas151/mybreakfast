    ...
    <section class="section-sm row bg-white row">
        ...
        <table>
            ...
            <tr>
                ...
                <td>
                    <div class="col-md-6">
                        <img src="images/vehicle/{{ $rbookings->vimage}}" alt="" width="370" height="244" />
                    </div>
                </td>
                <td>{{ $rbookings->vcidate}}</td>
                <td>{{ $rbookings->vcodate}}</td>
                <td>{{ $rbookings->vnop }}</td>
                <td>{{ $rbookings->vamount }}</td>
                <td>
                    @php
                    $todaycurrent = $today->format("m/d/Y")
                    @endphp
                    @if($rbookings->vcodate > $todaycurrent)
                    <a class="button button-icon button-primary cancel-vehicle" href="#" data-toggle="modal"
                        data-target="#cancelbook" data-vkid="{{ $rbookings->vbid }}"><span>Cancel</span><span
                            class="icon material-icons-chevron_right"></span></a></td>
        ...
        <table>
            ...
            <tr>
                ...
                <td>
                    <div class="col-md-6">
                        <img src="images/vehicle/{{ $obookings->vimage}}" alt="" width="370" height="244" />
                    </div>
                </td>
                <td>{{ $obookings->vcidate}}</td>
                <td>{{ $obookings->vcodate}}</td>
                <td>{{ $obookings->vnop }}</td>
                <td>{{ $obookings->vamount }}</td>
                <td>
                    @php
                    $todaycurrent = $today->format("m/d/Y")
                    @endphp
                    @if ($obookings->vcidate < $todaycurrent) <a
                        class="button button-icon button-primary rating-vehicle" href="#" data-toggle="modal"
                        data-target="#ratevahicle" data-vehid="{{ $obookings->vid }}"
                        data-vimager="{{ $obookings->vimage }}" data-vkrid="{{ $obookings->vbid }}">
                ...
                <div class="box">
                    <div class="content">
                        <div class="form loginBox">
                            <form method="post" action="/vbookcancel" accept-charset="UTF-8"
                                enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="bookvid" id="bookvid">
...
<div class="modal login" id="ratevahicle">
    <div class="modal-dialog login animated">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
            ...
            <div class="modal-body">
                <div class="box">
                    <div class="content">
                        <div class="form loginBox">
                            <form action="/ratingvehicle" method="post" accept-charset="UTF-8"
                                enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="vehicid" id="vehicid">
                                <input type="hidden" name="vbookid" id="vbookid">
                                <input id="ratings-hidden" name="rating" type="hidden" value="1">
                                <div class="col-md-6">
                                    {{-- <img src="" id="vrimagert" class="img-circle img-responsive"/> --}}
                                    <img src="" id="vrimagert" />
                                </div>
...
<script>
    $(".cancel-vehicle").on("click", function () {
        $bvkid = $(this).data("vkid");
        $("#bookvid").val($bvkid);
    });
    $(".rating-vehicle").on("click", function () {
        $vbrid = $(this).data("vehid");
        $("#vehicid").val($vbrid);
        $bvkid = $(this).data("vkrid");
        $("#vbookid").val($bvkid);
        $vrimage = $(this).data("vimager");
        $("#vrimagert").attr("src", "images/vehicle/" + $vrimage);
    });
