        ...
        <section>
            <div class="container-fluid"><br>
                <div class="text-center">
                    <h5 style="font-size:30px;">Edit Package</h5>
                </div>
                <div class="profile" align:="center">
                    <div class="swiper-container swiper-slider swiper-white" data-loop="false" data-autoplay="false"
                        ...
                        <form method="post" action="/savepackage" accept-charset="UTF-8" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="pkid" value="{{ $package[0]->pkid }}">
                            <label style="color:black; font-size: 17px;">Enter Your Package Name :</label>
                            <input type="text" class="form-control" value="{{ $package[0]->pkname }}" name="pname"
                            ...
                            <label style="color:black; font-size: 17px;">Enter Your Package Info :</label>
                            <input type="text" class="form-control" value="{{ $package[0]->pkinfo }}" name="pinfo"
                                id="pinfo" required onkeyup="capitalize(this.id, this.value);"
                            ...
                            <script>
                                function Validatepinfo() {
                                    ...
                                    if (!val.match(/^[A-Za-z][A-Za-z" "]{20,}$/)) {
                                        $("#pinfo_l").html('Min. 20 and Only Alphabets Allowed..!').fadeIn()
                                            .delay(4000).fadeOut();
                            ...
                            <label style="color:black; font-size: 17px;">Select a Room :</label>
                            <select type="text" id="roomid" name="roomid" class="form-control" required>
                                <option class="form-control" value="{{ $package[0]->nrid }}" disabled selected>
                                    {{ $package[0]->rname }}</option>
                                @foreach ($room as $rooms)
                                <option class="form-control" value="{{ $rooms->nrid }}">{{ $rooms->rname }}
                                </option>
                                @endforeach
                            </select>
                            <label style="color:black; font-size: 17px;">Select a Breakfast :</label>
                            <select type="text" id="breakfastid" name="breakfastid" class="form-control" required>
                                <option class="form-control" value="{{ $package[0]->bid }}" disabled selected>
                                    {{ $package[0]->bfname }}</option>
                                @foreach ($breakfast as $breakfasts)
                                <option class="form-control" value="{{ $breakfasts->bid }}">
                                    {{ $breakfasts->bfname }}</option>
                                @endforeach
                            </select>
                            <label style="color:black; font-size: 17px;">Select a Vehicle :</label>
                            <select type="text" id="vehicleid" name="vehicleid" class="form-control" required>
                                <option class="form-control" value="{{ $package[0]->vid }}" disabled selected>
                                    {{ $package[0]->vrno }}</option>
                                @foreach ($vehicle as $vehicles)
                                <option class="form-control" value="{{ $vehicles->vid }}">{{ $vehicles->vrno }}
                                </option>
                                @endforeach
                            </select>
                            <label style="color:black; font-size: 17px;">Package Starting Time :</label>
                            <input type="text" class="form-control" value="{{ $package[0]->pstime }}"
                                placeholder="Enter Package Starting Time" name="pstime" id="pstime" required
                                onchange="Validatestime()" ;>
                            ...
                            <label style="color:black; font-size: 17px;">Package Ending Time :</label>
                            <input type="text" class="form-control" value="{{ $package[0]->petime }}"
                                placeholder="Enter Package Ending Time" name="petime" id="petime" required
                                onchange="Validateetime()" ;>
                            ...
                            <label style="color:black; font-size: 17px;">Upload Package Image :</label>
                            <input type="file" value="{{ $package[0]->pkimage }}" class="form-control" id="pic"
                            ...
                            <label style="color:black; font-size: 17px;">Enter Package Amount :</label>
                            <input type="text" value="{{ $package[0]->rent }}" class="form-control"
                                placeholder="Enter Your Package Amount" name="amnt" id="amnt" required
